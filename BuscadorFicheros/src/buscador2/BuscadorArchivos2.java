...
public class BuscadorArchivos2 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					BuscadorArchivos2 frame = new BuscadorArchivos2();
	...
	public BuscadorArchivos2() {
		...
		JLabel lblTamao = new JLabel("DD-MM-AAAA");
		lblTamao.setBounds(6, 59, 123, 15);
		contentPane.add(lblTamao);

		textField = new JTextField();
		textField.setBounds(92, 53, 100, 27);
		contentPane.add(textField);
		textField.setColumns(10);

		JRadioButton rdbtnMenor = new JRadioButton("DESPUÉS DE");
		buttonGroup.add(rdbtnMenor);
		rdbtnMenor.setBounds(206, 87, 113, 18);
		contentPane.add(rdbtnMenor);

		rdbtnMayor = new JRadioButton("ANTES DE");
		buttonGroup.add(rdbtnMayor);
		rdbtnMayor.setBounds(206, 57, 113, 18);
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					for (File f : encuentros) {
						texto += f.getPath() + "(Última modificación: " + sdf.format(new Date(f.lastModified()))
								+ ")\n";
					}
					textArea.setText(texto);
				} catch (ParseException pe) {
					JOptionPane.showMessageDialog(null,
							"Tamaño no válido\n\n\n" + pe.getClass() + " " + pe.getMessage());
				}
			}
